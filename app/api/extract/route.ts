...
async function extractOne(file: File, attempt = 1): Promise<any> {
  try {
    ...
    const result = await model.generateContent([
      ...
      {
        text: "Extract the property address and details of every agency marketing this property, including joint/co-agency listings where two or more separate companies are involved. For each agency, extract its name and every individual contact person listed for that agency (name, email, phone). The address MUST include the full UK postcode (e.g. EC4N 8AD, not just EC4) - search the ENTIRE document exhaustively for this: check the header, footer, small print, disclaimer text, floor plans, EPC certificates, and any legal/agent contact blocks, since the full postcode is very often printed in a different location than the main address heading. Only return a partial postcode (like just 'EC4') if you have thoroughly checked the whole document and a full postcode genuinely does not appear anywhere. If a field genuinely isn't present, return null for it - do not guess or invent values.",
      },
    ]);
